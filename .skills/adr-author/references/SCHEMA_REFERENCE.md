...
13. `adr.y_statement` should target **100–150 words**. It is the most information-dense *summary* of the decision — not a second rationale. Each of the 7 clauses should be concise: use just names in `neglected`, one headline in `facing`, top 2 tradeoffs in `accepting`, and one core reason in `because`. Detail belongs in the dedicated sections (`context.description`, `decision.rationale`, `decision.tradeoffs`, `consequences`). **Bold the 7 clause keywords** (`**In the context of**`, `**facing**`, etc.) to aid scanning. Always use `**and neglected**` — never "and rejected."
14. `consequences.positive[]` and `consequences.negative[]` should target **15–25 words per item**. Each item states a single clear outcome — not a paragraph. Explanatory detail belongs in `decision.rationale` or `decision.tradeoffs`.
15. `audit_trail[].details` should target **15–30 words per entry**. Audit trail entries are log entries, not essays. Format: "What happened + one sentence of significance."
...
⚠️ **YAML scalar pitfalls** — four common issues can silently corrupt ADR content:
...
4. **`$:` in plain scalars is a mapping value** — The `$:` token (Svelte's `$:` labels, jQuery selectors) is parsed as a YAML mapping key. Use `>-` for list items containing `$:`.
