...
4. **Complementary to existing summaries.** Our `adr.summary` field is a free-text elevator pitch. The Y-Statement is a *structured* summary that ensures consistent coverage of all six elements. Both serve different purposes.
...
|---|---|---|
| **Short form** | "In the context of..., facing..., we decided for... and against..., to achieve..., accepting..." | Chat messages, ADR index tables, dashboard widgets |
| **Long form** | Short form + "because..." clause | Rendered ADR documents, stakeholder emails, meeting summaries |

The "because" clause maps to `decision.rationale` — the most important field to surface for decision understanding.

#### Implementation approach: AI capability + script + rendering

The original proposal offered three mutually exclusive options. After analysis, **the best approach combines elements of all three**:

| Component | What | Why |
|---|---|---|
| **AI Bundle capability** (Option A) | Add as capability 8: "Generate Y-Statement" | For web chat users without repo access — the AI synthesizes from context |
| **Summarize script enhancement** (Option B) | Add `--format y-statement` to `summarize-adr.py` | Deterministic, reproducible, no AI needed — ideal for CI/CD and batch processing |
| **Rendered Markdown section** (new) | Add Y-Statement block to `render-adr.py` output | Every rendered ADR automatically includes a Y-Statement for quick comprehension |

**Not recommended:** Option C (schema field `adr.y_statement`). The Y-Statement is a **derived view**, not an authored field. Storing it in the schema would create a maintenance burden (keeping it in sync) and violate DRY. It should always be rendered on-the-fly from the constituent schema fields.
...
- [ ] Add `--format y-statement` to `scripts/summarize-adr.py` (deterministic template rendering)
- [ ] Add Y-Statement section to `scripts/render-adr.py` output (auto-generated in every rendered ADR)
- [ ] Add Y-Statement generation instructions to `repomix-instruction.md` (capability 8)
- [ ] Update `.skills/adr-author/SKILL.md` to reference Y-Statement as a post-authoring output
