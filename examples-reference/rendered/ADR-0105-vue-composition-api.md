...
> ***In the context of** Vue.js 3 API design, **facing** code organization fragmentation in complex components where related logic is scattered across multiple options blocks, **we decided for** the Composition API — a function-based setup() entry point with reactive primitives (ref, reactive, computed, watch) that coexists with the Options API — **and neglected** maintaining the Options API as the sole API, a Class-based API with decorators, the React Hooks model, and mixins-only logic reuse, **to achieve** function-based code organization by logical concern, clean composable logic extraction without namespace collisions, and full TypeScript type inference, **accepting** the mental overhead of ref vs. reactive and .value unwrapping, and community backlash from developers who perceived the change as abandoning Vue's simplicity, **because** function composition with explicit reactive primitives was the only approach solving code organization, logic reuse, and type inference simultaneously without relying on unstable TC39 proposals.*
