...
> *In the context of ESLint's configuration architecture, facing a 9-year accumulation of incremental complexity in the eslintrc system — including cascading directory configs, multiple file formats (JSON, YAML, JS), custom require resolution that broke when npm v3 stopped installing peer dependencies, confusing extends-inside-overrides merging, and environment- based globals that conflicted with ecmaVersion settings — we decided for a new flat config system (eslint.config.js) that uses native JavaScript module loading, exports a single array of config objects with glob-based file matching, and eliminates the directory cascade entirely, and neglected keeping the eslintrc system with incremental fixes, adopting a YAML-only declarative config format, adopting TOML as the configuration language, exposing a full programmatic API without any file convention, and relying solely on the FlatCompat compatibility shim without a full system redesign, to achieve a single predictable configuration file per project, native ES module loading that eliminates custom require resolution, explicit plugin imports as JavaScript objects instead of magic string resolution, glob-based file matching that replaces both the directory cascade and the overrides key, and modern defaults (ecmaVersion latest, sourceType module), accepting that every existing eslintrc configuration must be migrated, that the plugin ecosystem needed time to adopt the new format, and that the 18-month RFC debate nearly split the core team, because the eslintrc system had reached a level of complexity where even the core team was afraid to touch the config code, the custom require resolution was the single biggest source of user support requests, and incremental fixes could not address the fundamental architectural problems of cascading configs and multiple file formats.*
