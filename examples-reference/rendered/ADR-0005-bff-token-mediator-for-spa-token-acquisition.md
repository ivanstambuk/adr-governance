...
<!-- Source of truth: ../fictional/ADR-0005-bff-token-mediator-for-spa-token-acquisition.yaml -->

> [!CAUTION]
> **This file is auto-generated** from [`ADR-0005-bff-token-mediator-for-spa-token-acquisition.yaml`](../fictional/ADR-0005-bff-token-mediator-for-spa-token-acquisition.yaml).
> Do not edit this file directly — all changes must be made in the YAML source.

# ADR-0005-bff-token-mediator-for-spa-token-acquisition: Use the Backend-for-Frontend token mediator pattern over direct SPA-to-AS authorization for single-page applications
...
> *In the context of single-page application (SPA) OAuth 2.1 integration, facing the inability to securely store tokens in browser environments and the risk of XSS-based token exfiltration, we decided for the Backend-for-Frontend (BFF) token mediator pattern and neglected direct SPA-to-authorization-server flows with in-memory tokens and service worker token caches with encrypted storage, to achieve server-side token storage with httpOnly/Secure/SameSite cookie-based session binding that eliminates browser-accessible token material, accepting the operational overhead of deploying and scaling a BFF service per SPA and increased backend latency for token exchange, because the BFF pattern is the only approach that fully removes tokens from the browser attack surface while maintaining OAuth 2.1 compliance per the Security BCP.*
